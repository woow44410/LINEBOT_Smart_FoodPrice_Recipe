...
def arrowedLine(im, ptA, ptB, width=1, fill=(255,255,255)):
...
def getIngredientReportPNG(ingreID,ingreName,userID):
    ...
    for i,price in enumerate(pricePercentageList):
        ...
        if i != 0:
            ...
            tmpTextYAxis = (preY2Axis+avgYAxis)/2
            # 大於等於0畫白線 小於0畫紅線
            if avgPriceList[i] - preAvgPrice >=0:
                ...
                if y2-10 -preY2Axis < -75:
                    draw.line((preX2Axis,preY2Axis+10,x2-50,y2+12),fill=redColor,width=7)
                    arrowedLine(bk_img,(preX2Axis,preY2Axis+10),(x2-40,y2+1),fill = redColor)
                    draw.text((avgXAxis-57,avgYAxis+20 ),"{:<5.2f}元".format(tmpAvgPrice),font = fontPrice,fill=redColor)
                elif y2-10 - preY2Axis < -30:
                    draw.line((preX2Axis,preY2Axis+10,x2-50,y2+6),fill=redColor,width=7)
                    arrowedLine(bk_img,(preX2Axis,preY2Axis+10),(x2-40,y2+1),fill = redColor)
                    draw.text((avgXAxis-60,avgYAxis+20 ),"{:<5.2f}元".format(tmpAvgPrice),font = fontPrice,fill=redColor)
                elif y2-10 < preY2Axis :
                    draw.line((preX2Axis,preY2Axis+10,x2-50,y2+6),fill=redColor,width=7)
                    arrowedLine(bk_img,(preX2Axis,preY2Axis+10),(x2-40,y2+1),fill = redColor)
                    draw.text((avgXAxis-30,avgYAxis+15 ),"{:<5.2f}元".format(tmpAvgPrice),font = fontPrice,fill=redColor)
                else:
                    draw.line((preX2Axis,preY2Axis+12,x2-50,y2),fill=redColor,width=7)
                    arrowedLine(bk_img,(preX2Axis,preY2Axis+10),(x2-40,y2),fill = redColor)
                    draw.text((avgXAxis-20,avgYAxis+10 ),"{:<5.2f}元".format(tmpAvgPrice),font = fontPrice,fill=redColor)
            else:
                if y2-10 - preY2Axis >75:
                    draw.line((preX2Axis,preY2Axis+13,x2-50,y2-8),fill=whiteColor,width=7)
                    arrowedLine(bk_img,(preX2Axis,preY2Axis+10),(x2-40,y2))
                    draw.text((avgXAxis+27,tmpTextYAxis+20),"{:<5.2f}元".format(tmpAvgPrice),font = fontPrice,fill=whiteColor)
                elif y2-10 > preY2Axis:
                    draw.line((preX2Axis,preY2Axis+13,x2-50,y2-5),fill=whiteColor,width=7)
                    arrowedLine(bk_img,(preX2Axis,preY2Axis+10),(x2-40,y2))
                    draw.text((avgXAxis+17,avgYAxis+15),"{:<5.2f}元".format(tmpAvgPrice),font = fontPrice,fill=whiteColor)
                else:
                    draw.line((preX2Axis,preY2Axis+13,x2-50,y2+1),fill=whiteColor,width=7)
                    arrowedLine(bk_img,(preX2Axis,preY2Axis+10),(x2-40,y2+1))
                    draw.text((avgXAxis-23,tmpTextYAxis-10),"{:<5.2f}元".format(tmpAvgPrice),font = fontPrice,fill=whiteColor)
    ...
    # # 展示出我X媽花六個小時畫的圖片
    #bk_img.show()
#getIngredientReportPNG("LA1","甘藍-初秋","u004")
#getIngredientReportPNG("LA2","甘藍-改良種","u004")